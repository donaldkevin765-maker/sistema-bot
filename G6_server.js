...
const { G6BotManager, SkillTracker } = require('./G6_bot.js');
...
function getOrCreateRoom(n) {
  const f = ROOM_PREFIX + n;
  if (!rooms.has(f)) {
    const room = new GameRoom(f);
    room.botManager = new G6BotManager(room, skillTracker);
    rooms.set(f, room);
  }
  return rooms.get(f);
}
